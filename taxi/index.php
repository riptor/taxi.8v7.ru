...
if (isset($_GET['date'])) {
$query = "SELECT * FROM $userstable WHERE vipolneno = 'Нет' ORder by date DESC"; 
...
$query = "SELECT * FROM $userstable WHERE vipolneno = 'Нет' ORder by date"; 
/* Выполнить запрос. Если произойдет ошибка - вывести ее. */ 
$res = mysqli_query($link, $query); 
}
...
if ($id == 0) { 
  ...
  echo "<TR>
 	 <TD>".$row ['date']."&nbsp</TD><TD>".$row ['adres001']."&nbsp</TD><TD>".$row ['paradnaya']."&nbsp</TD><TD>".$row ['adres002']."&nbsp</TD><TD>".$row ['mani']."&nbsp</TD><TD><a href='./izmenen.php?id=".$row ['id']."'><img src='../png/tool.gif'alt='Изменить'border='0'></a></TD><TD>
